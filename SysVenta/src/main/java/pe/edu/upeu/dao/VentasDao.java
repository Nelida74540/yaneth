...
public class VentasDao extends AppCrud{
    ...
    public void reporteVentas() {
        ...
        leerA=new LeerArchivo(TABLA_VENTA);
        Object[][] dataP=listarContenido(leerA);
        String fechaI=leerT.leer("", "Ingrese fecha inicio");
        String fechaF=leerT.leer("", "Ingrese fecha fin");
        int cantidadFilas=0;
        try {
            for (int i = 0; i < dataP.length; i++){
                ...
                if(
                    ...
                    (fechaV.before(formatFecha.parse(fechaI)) || fechaSeparada[0].equals(fechaI))
                    ){
                        cantidadFilas++;
            ...
            System.out.printf("%d.%s %5s %10s %d %3d %3d %-4d ","#","IdV","DNI",
            "Fecha","Desct.","P.Base","IGV","P.Total");
            int num=0;
            for (VentaTO ventaTO : ventasERF) {
                System.out.printf("d %s %5s %10s %d %3d %3d %4d %n",
